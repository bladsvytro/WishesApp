...
class SpaceRepository @Inject constructor(
    ...
    suspend fun joinSpace(inviteCode: String, displayName: String): Space {
        ...
        if (query.isEmpty) error("Invite code not found")
        val doc = query.documents.first()
        val space = doc.toObject(Space::class.java) ?: error("Malformed space document")
        if (space.members.size >= 2) error("This space is already full")
        if (space.members.containsKey(uid)) error("You are already in this space")
